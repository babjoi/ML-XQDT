...
import org.eclipse.wst.rng.contentassist.RngTools;
...
import org.eclipse.wst.rng.core.internal.binding.BindingUtils;
...
public class ChangeSchemaAssociationCommandHandler extends AbstractEnabledHandler {
	...
	private void handleRemoveAssociation(XMLMultiPageEditorPart editor, IFileEditorInput currInput) {
		...
		if (domModel == null) {
			return; // cannot proceed :( TODO: inform user
		}
		
		findAndDestroyObsoleteInstructions(domModel);
		
		// do not change model content type when there's still a match in binding catalogs
	...
	private void handleNewAssociation(XMLMultiPageEditorPart editor, IFileEditorInput currInput) {
		...
		if (schemaUri == null) {
			return; // OK, user changed his mind
		}
	...
	private void insertProcessingInstruction(Document document, String schemaUri) {
		...
		if (schemaSyntax == null) {
			return; // TODO : prompt user to manually specify the syntax
		}
		String instructionData = RngConstants.BINDING_INSTRUCTION_URI_PSEUDO_ATT_NAME + "=\"" + schemaUri + "\" " +
			RngConstants.BINDING_INSTRUCTION_TYPE_PSEUDO_ATT_NAME + "=\"" + BindingUtils.convertSchemaSyntaxToMimeType(schemaSyntax) + "\" " +
			RngConstants.BINDING_INSTRUCTION_MODE_PSEUDO_ATT_NAME + "=\"" + RngConstants.BINDING_INSTRUCTION_MODE_PSEUDO_ATT_VALUE + "\"";
		...
		for (int i = 0; i < docChildren.getLength(); i++) {
			...
			if (item instanceof ProcessingInstruction) {
				...
				if (RngConstants.BINDING_INSTRUCTION_TARGET.equals(pi.getTarget())) {
					...
					if (attributes.containsKey(RngConstants.BINDING_INSTRUCTION_MODE_PSEUDO_ATT_NAME) &&
							attributes.get(RngConstants.BINDING_INSTRUCTION_MODE_PSEUDO_ATT_NAME).equals(RngConstants.BINDING_INSTRUCTION_MODE_PSEUDO_ATT_VALUE)) {
						document.removeChild(item);
					}					
				}
			} else if (item instanceof Element) {
				break; // do not take processing instructions after root element into account
			}
		}
	}
