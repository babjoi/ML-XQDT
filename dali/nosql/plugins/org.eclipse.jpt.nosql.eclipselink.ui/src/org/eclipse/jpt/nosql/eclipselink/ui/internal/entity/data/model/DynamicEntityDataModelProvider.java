 ...
 ******************************************************************************/
package org.eclipse.jpt.nosql.eclipselink.ui.internal.entity.data.model;
...
import org.eclipse.jpt.nosql.eclipselink.ui.internal.entity.data.operation.NewDynamicEntityClassOperation;
...
public class DynamicEntityDataModelProvider extends NewJavaClassDataModelProvider implements IEntityDataModelProperties {
	...
	private IStatus validateJavaPackage(String packName) {		
		if (packName == null || packName.equals(EMPTY_STRING)) {
			return JptNoSQLEclipseLinkUiPlugin.instance().buildStatus(IStatus.WARNING, EclipseLinkUiMessages.DynamicEntityClassWizardPage_defaultPackageWarning);
		}			
	...
	private IStatus validateXmlName(String xmlName) {
		...
		if (project != null) {
			...
			if (ormXmlResource == null) {
				return JptNoSQLEclipseLinkUiPlugin.instance().buildErrorStatus(EclipseLinkUiMessages.DynamicEntityClassWizardPage_invalidXMLName);
			}
			else if (this.getJpaProject().getJpaFile(ormXmlResource.getFile()).getRootStructureNodesSize() == 0) {
				return JptNoSQLEclipseLinkUiPlugin.instance().buildErrorStatus(EclipseLinkUiMessages.DynamicEntityClassWizardPage_xmlNotListedError);
			}
		}
		return Status.OK_STATUS;
	}
	...
	private IStatus validateFieldsList(ArrayList<DynamicEntityField> fields) {
		if (fields != null && !fields.isEmpty()) {
			...
			if (hasDuplicates) {
				return JptNoSQLEclipseLinkUiPlugin.instance().buildErrorStatus(EclipseLinkUiMessages.DynamicEntityFieldsWizardPage_duplicateEntityFieldsError);
			}
			...
			if (errorMsg1 != null) {
				return JptNoSQLEclipseLinkUiPlugin.instance().buildErrorStatus(errorMsg1);
			}
			...
			if (errorMsg2 != null) {
				return JptNoSQLEclipseLinkUiPlugin.instance().buildErrorStatus(errorMsg2);
			}
			...
			if (errorMsg3 != null) {
				return JptNoSQLEclipseLinkUiPlugin.instance().buildErrorStatus(errorMsg3);
			}
			...
			if (warningMsg1 != null) {
				return JptNoSQLEclipseLinkUiPlugin.instance().buildWarningStatus(warningMsg1);
			}
			...
			if (warningMsg2 != null) {
				return JptNoSQLEclipseLinkUiPlugin.instance().buildWarningStatus(warningMsg2);
			}
		}
		return Status.OK_STATUS;
	}
	...
	private String checkInputFieldsMappingTypes(ArrayList<DynamicEntityField> fields) {
		...
		if (hasIDAndEmbeddedIDMappingDefined(mappingKeys)) {
			validateFieldMappingTypeStatus = JptNoSQLEclipseLinkUiPlugin.instance().buildErrorStatus(EclipseLinkUiMessages.EclipseLinkDynamicEntityWizard_bothIDAndEmbeddedIDDefinedError);
		} else if (hasMultipleEmbeddedIDMappings(mappingKeys)) {
			validateFieldMappingTypeStatus = JptNoSQLEclipseLinkUiPlugin.instance().buildErrorStatus(EclipseLinkUiMessages.EclipseLinkDynamicEntityWizard_multipleEmbeddedIDsDefinedError);
		}
	...
	private String checkInputFieldsAttributeTypeValidity(List<DynamicEntityField> fields) {
		...
		for (DynamicEntityField field: fields) {
			...
			if (field.isKey() && !field.couldTypeBePKType()) {
				validateFieldTypeStatus = JptNoSQLEclipseLinkUiPlugin.instance().buildErrorStatus(EclipseLinkUiMessages.EclipseLinkDynamicEntityWizard_invalidPKType, field.getFqnAttributeType());
				break;				
			}
			String sig = null;
			try {
				...
				validateFieldTypeStatus = JptNoSQLEclipseLinkUiPlugin.instance().buildErrorStatus(EclipseLinkUiMessages.EclipseLinkDynamicEntityWizard_invalidArgument, e.getLocalizedMessage());
				break;
			}
			if (sig == null) {
	...
	protected String checkInputFieldsAttributeTypeExistence(List<DynamicEntityField> fields) {
		...
		for (DynamicEntityField field: fields) {
			...
			if (sig == null) {
				validateFieldTypeStatus = JptNoSQLEclipseLinkUiPlugin.instance().buildErrorStatus(EclipseLinkUiMessages.EclipseLinkDynamicEntityWizard_typeNotInProjectClasspath, field.getFqnAttributeType());
				break;
			}
			int sigType = Signature.getTypeSignatureKind(sig);
			if (sigType == Signature.BASE_TYPE_SIGNATURE){
	...
	protected String checkInputFieldsTargetTypeExistence(List<DynamicEntityField> fields) {
		...
		for (DynamicEntityField field: fields) {
			...
			if (sig == null) {
				validateFieldTypeStatus = JptNoSQLEclipseLinkUiPlugin.instance().buildErrorStatus(EclipseLinkUiMessages.EclipseLinkDynamicEntityWizard_typeNotInProjectClasspath, field.getFqnTargetType());
				break;
			}
			int sigType = Signature.getTypeSignatureKind(sig);
			if (sigType == Signature.BASE_TYPE_SIGNATURE){
	...
	private IStatus validatePrimaryKeyFieldsList(ArrayList<DynamicEntityField> pkFields) {
		return (pkFields.size() > 1) ?
				JptNoSQLEclipseLinkUiPlugin.instance().buildStatus(IStatus.INFO, EclipseLinkUiMessages.EclipseLinkDynamicEntityWizard_applyEmbeddedIdMappingInfo) :
				null;
	}
	...
	protected IStatus canCreateTypeInClasspath(String packageName, String typeName) {
		...
		for (String name : this.getJavaTypeNames()) {
			if (ObjectTools.equals(name, fullyQualifiedName)) {
				return JptNoSQLEclipseLinkUiPlugin.instance().buildStatus(IStatus.WARNING, EclipseLinkUiMessages.EclipseLinkDynamicEntityWizard_typeExistsWarning, fullyQualifiedName);

			} else if (StringTools.equalsIgnoreCase(name, fullyQualifiedName)) {
				return JptNoSQLEclipseLinkUiPlugin.instance().buildStatus(IStatus.WARNING, EclipseLinkUiMessages.EclipseLinkDynamicEntityWizard_typeWithDiffCaseExistsWarning, fullyQualifiedName);
			}
		}
		...
		if (pu != null) {
			for (String name : ((EclipseLinkPersistenceUnit)this.getPersistenceUnit()).getEclipseLinkDynamicPersistentTypeNames()) {
				if (ObjectTools.equals(name, fullyQualifiedName)) {
					return JptNoSQLEclipseLinkUiPlugin.instance().buildErrorStatus(EclipseLinkUiMessages.EclipseLinkDynamicEntityWizard_dynamicTypeExistsError, fullyQualifiedName);

				} else if (StringTools.equalsIgnoreCase(name, fullyQualifiedName)) {
					return JptNoSQLEclipseLinkUiPlugin.instance().buildErrorStatus(EclipseLinkUiMessages.EclipseLinkDynamicEntityWizard_dynamicTypeWithDiffCaseExistsError, fullyQualifiedName);
				}
			}
		} else {
			return JptNoSQLEclipseLinkUiPlugin.instance().buildErrorStatus(EclipseLinkUiMessages.EclipseLinkDynamicEntityWizard_persistenceUnitNotFoundError);
		}
		return Status.OK_STATUS;
	}
