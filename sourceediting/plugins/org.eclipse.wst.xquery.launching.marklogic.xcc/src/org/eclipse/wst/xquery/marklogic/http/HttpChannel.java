...
import org.eclipse.wst.xquery.marklogic.io.LengthLimitedInputStream;
...
public class HttpChannel {
    ...
    public HttpChannel(ByteChannel channel, String method, String path, int bufferSize, int timeoutMillis, Logger logger) {
        ...
        requestHeaders.setRequestValues(method, path, "HTTP/1.1");

        if (logger == null) {
            logger = Logger.getLogger(getClass().getName());
        }

        logger.fine("HTTP request: " + requestHeaders.getRequestLine());
